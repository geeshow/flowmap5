...
```bash
python3 -m http.server 8770 --directory .   # 저장소 루트를 서빙 (아무 정적 서버나 가능)
# → http://localhost:8770/
```

빌드 도구나 의존성이 없습니다. `index.html`(저장소 루트) + `web/` 디렉토리(`app.js` + `style.css` +
`features/` + `data/`)가 전부이며, `index.html` 의 `<base href="web/">` 가 상대경로를 `web/` 아래로 해석합니다.
...
```
graphs/<service>.json  ──  scripts/build.py  ──▶  web/data/graph.json  ──▶  브라우저
        ▲                                              (id 기준 union)
        │
...
# 1) 두 분석기 json/ 의 per-project 파일을 web/data/ 로 복사 (_* 통합본 제외)
