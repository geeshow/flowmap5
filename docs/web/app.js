...
function attachHandlers() {
  ...
  document.querySelectorAll('#nav .nav-btn[data-view]').forEach(b =>
    b.addEventListener('click', () => state.view === b.dataset.view ? setOverview(true) : openView(b.dataset.view)));
  setupDetailResizer();
...
function fetchData(path) {
  if (!dataCache.has(path))
    dataCache.set(path, fetch(path).then(r => r.ok ? r.json() : null).catch(() => null));
...
function openView(view, params) {
  ...
  state.view = view;
  state.overview = false; state.service = null; state.infraType = null; state.svcPick = null;
