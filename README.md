...
```bash
# 1) 서비스별 그래프(graphs/*.json)를 하나로 머지 → web/data/graph.json
python3 scripts/build.py

# 2) 정적 서버로 띄우기 (아무 정적 서버나 가능)
python3 -m http.server 8770
# → http://localhost:8770/web/
```
...
├── scripts/
│   └── build.py        # graphs/*.json 머지 → web/data/graph.json
└── web/                # 시각화 웹 앱 (바닐라 JS, 의존성 없음)
    ├── index.html
    ├── app.js
    ├── style.css
...
call-graph-spring-kotlin (정적 분석)
        ...
        ▼
graphs/<service>.json  ──  scripts/build.py (id 기준 union 머지)  ──▶  web/data/graph.json
                                                                         │
                                                                         ▼
                                                                   web/ (브라우저 렌더링)
```
