...
const state = {
  ...
  svcPathRepo: null,         // 경로 드릴 출처가 repo 포커스면 그 repo — 브레드크럼 중간 링크 복귀 대상
  ...
  fromSvcPath: null,         // 경로 드릴(3단계)에서 노드를 눌러 호출관계분석(4단계)으로 들어온 경우 그 경로 키
  fromSvcRepo: null,         // 그 경로 드릴의 출처 repo(있으면) — 4단계 브레드크럼 복귀용
...
function parseUrl() {
  ...
  const rf = rawParam('rf');                                              // 경로 드릴 출처 repo (repo 포커스 복귀용)
  state.svcPathRepo = state.svcPath && rf && repoList().includes(decodeURIComponent(rf)) ? decodeURIComponent(rf) : null;
  ...
  // 4단계(호출관계분석)가 경로 드릴에서 왔으면 그 경로 키·repo 복원(브레드크럼 체인용)
  state.fromSvcPath = state.focus && state.fromService && rawParam('fpp') ? decodeURIComponent(rawParam('fpp')) : null;
  const frf = rawParam('frf');
  state.fromSvcRepo = state.fromSvcPath && frf && repoList().includes(decodeURIComponent(frf)) ? decodeURIComponent(frf) : null;
...
function pushUrl() {
  ...
  if (state.service || state.infraType) {
    ...
    if (state.service && state.svcPath && state.svcPathRepo) parts.push('rf=' + encodeURIComponent(state.svcPathRepo));   // 출처 repo 포커스
  ...
  if (state.focus) {
    ...
    if (state.fromService && state.fromSvcPath) parts.push('fpp=' + encodeURIComponent(state.fromSvcPath));
    if (state.fromService && state.fromSvcPath && state.fromSvcRepo) parts.push('frf=' + encodeURIComponent(state.fromSvcRepo));
...
function setFocusFromOverview(id) {            // 전체보기 인프라/외부 노드 → 호출관계분석 (전체보기 › 노드)
  if (!nodeById.has(id)) return;
  state.view = null;
  state.fromOverview = true; state.fromService = null; state.fromSvcPath = null; state.fromSvcRepo = null;
...
//   fromRepo: repo 포커스(전체보기 repo 한정)의 컴포넌트 카드에서 들어왔으면 그 repo — 브레드크럼 복귀 대상.
function setSvcPath(svc, path, fromRepo) {
  if (!svc || !path) return;
  state.view = null;
  state.service = svc; state.svcPath = path; state.svcPick = null; state.sel = null;
  state.svcPathRepo = fromRepo || null;
...
function setFocus(id, origin) {                // 노드 기준 호출관계분석
  if (!nodeById.has(id)) return;
  state.view = null;
  if (origin !== undefined) { state.fromService = origin || null; state.fromOverview = false; state.fromSvcPath = null; state.fromSvcRepo = null; }
  state.focus = id; state.sel = id; state.expanded = false; state.overview = false; state.structure = false; state.service = null; state.infraType = null; state.svcPick = null;
  pushUrl(); render(); renderDetail();
}
// 4단계: 경로 드릴(3단계)에서 노드 클릭 / 검색 진입 → 그 노드 기준 호출관계분석. ctx 미지정 시 현재 경로 드릴 상태 사용.
function setFocusFromPath(id, ctx) {
  if (!nodeById.has(id)) return;
  ctx = ctx || { svc: state.service, path: state.svcPath, repo: state.svcPathRepo };
  if (!ctx.svc || !ctx.path) { setFocus(id, null); return; }   // 경로 컨텍스트 없으면 일반 호출관계분석
  state.view = null;
  state.fromService = ctx.svc; state.fromSvcPath = ctx.path; state.fromSvcRepo = ctx.repo || null; state.fromOverview = false;
  state.focus = id; state.sel = id; state.expanded = false;
  state.overview = false; state.structure = false; state.service = null; state.svcPath = null; state.svcPathRepo = null; state.infraType = null; state.svcPick = null;
  state.up = 2; state.down = 2;
  pushUrl(); render(); renderDetail();
}
// 노드 → 4단계 진입에 쓸 경로 드릴 컨텍스트(서비스/경로키/repo). 인프라·외부호출·프로젝트 없는 노드는 null.
function focusPathCtxOf(n) {
  if (!n || !n.project || isInfra(n.id, n) || isExtCallNode(n.id, n)) return null;
  return { svc: n.project, path: compKeyOfNode(n) || pathKeyOf(n), repo: repoOf(n.project) };
}
function clearFocus() {
  state.view = null;
  state.focus = null; state.expanded = false; state.overview = false; state.structure = false; state.service = null; state.infraType = null; state.svcPick = null; state.fromService = null; state.fromSvcPath = null; state.fromSvcRepo = null; state.fromOverview = false;
  state.sel = null;
  pushUrl(); render(); renderDetail();
}
...
function render() {
  ...
  if (!state.svcPath) state.svcPathRepo = null;   // 경로 드릴을 벗어나면 출처 repo 기억 해제
  ...
  else if (state.focus && nodeById.has(state.focus)) (state.fromSvcPath ? renderFocusPathFlow : renderAnalysis)();
  else renderOverview();
  renderProcessDock();
}
...
function renderProcessDock() {
  ...
  // 서비스 보기 = svcPick 기준(현재 컬럼 엣지), 기능 뷰/4단계 호출관계분석 = 선택 노드(state.sel) 기준(실제 그래프 체인)
  const svcMode = !!state.service;
  const focusMode = !state.service && !!state.focus;   // 호출관계분석(4단계) — 선택 노드의 전체 그래프 체인
  const base = svcMode ? state.svcPick : ((dockFeature || focusMode) ? state.sel : null);
...
function renderServicePathDrill() {
  ...
  // key 는 진입 경로에 따라 두 가지: 서비스 보기는 pathKeyOf(엔드포인트 경로), repo 포커스 컴포넌트
  //   카드는 compKeyOfNode(.biz. 컴포넌트 또는 Gradle module). 둘 다 받아야 module 키(예: shopflow-order)로
  //   들어와도 노드가 잡힌다(두 키 네임스페이스는 겹치지 않아 오매칭 없음).
  const baseEps = serviceEndpoints(svc).filter(n => pathKeyOf(n) === key || compKeyOfNode(n) === key);   // 화면 또는 API 엔드포인트
  ...
  // 출처에 맞게 중간 링크 복귀: repo 포커스에서 들어왔으면 repo 포커스로, 아니면 서비스 보기로.
  //   repo≠svc(모노레포: nexcore/acc-app-ac)면 repo·svc 크럼을 모두 노출해 각자 제자리로 보낸다.
  const repo = state.svcPathRepo;
  const bc = document.getElementById('breadcrumb');
  bc.style.display = 'flex';
  let html = `<a class="bc-link" id="bc-root">🗺️ 전체보기</a><span class="bc-sep">›</span>`;
  if (repo && repo !== svc)
    html += `<a class="bc-link" id="bc-repo">${svcBadge(repo)}</a><span class="bc-sep">›</span>`;
  html += `<a class="bc-link" id="bc-svc">${svcBadge(svc)}</a>`
    + `<span class="bc-sep">›</span><span class="bc-focus">${esc(key)}</span>`;
  bc.innerHTML = html;
  bc.querySelector('#bc-root').addEventListener('click', () => setOverview(true));
  bc.querySelector('#bc-repo')?.addEventListener('click', () => setOverview(true, repo));
  // svc 크럼: repo 포커스 출처면(그리고 repo===svc 면) repo 포커스로, 아니면 서비스 보기로.
  bc.querySelector('#bc-svc').addEventListener('click', () =>
    repo && repo === svc ? setOverview(true, repo) : setService(svc));

  // 노드 클릭 → 4단계: 그 노드 기준 호출관계분석(연결된 경계 노드 + 하단 프로세스 흐름 + 우측 상세).
  //   다른 프로젝트의 노드를 눌러도 같은 경로 트레일(전체보기›repo›경로) 위에서 그 노드를 중심으로 펼친다.
  renderBoundaryFlow(svc, members, id => setFocusFromPath(id));
}

// 경계 노드 판별 — 화면 / endpoint(컨트롤러) / 외부·인프라(DB 제외). 3·4단계 공통.
function isBoundaryNode(id) {
  const n = nodeById.get(id);
  return !!n && (n.layer === 'SCREEN' || n.layer === 'CONTROLLER' || (isInfra(id, n) && infraGroup(id) !== 'db'));
}

// 경계(화면/endpoint/외부·인프라) 노드만 서비스별 컬럼으로 그린다 — 3단계(경로 그룹)·4단계(focus) 공통.
//   members = 기준 노드들(항상 렌더). 내부 실행 체인(service/repository/component…)은 컬럼에서 접고
//   경계끼리 직접 잇는다(클릭 시 하단 프로세스 독에 펼쳐짐). onPick = 노드 클릭 핸들러.
function renderBoundaryFlow(svc, members, onPick) {
  const colsEl = document.getElementById('columns');
  colsEl.className = 'struct-flow';
  colsEl.innerHTML = '';
  if (!members.length) {
    colsEl.innerHTML = `<div class="browse-empty">표시할 노드를 찾을 수 없습니다.</div>`;
    currentEdges = []; buildCurrentAdj();
    requestAnimationFrame(() => { document.getElementById('connectors').innerHTML = ''; });
    return;
  }
  const baseSet = new Set(members);

  // 연관 폐포: 다운스트림(실행 흐름 전체) + 업스트림(이 노드를 부르는 화면/타서비스 엔드포인트까지만).
  ...
  const { nodes, segOf, segLabels } = collectChainFlow(members, aOut, aIn);
  ...
  //   기준(members)은 내부 레이어여도 항상 그린다(focus 노드 보장).
  const renderable = id => baseSet.has(id) || (segOf.has(id) && reach.has(id) && isBoundaryNode(id));
  ...
  for (let s = 0; s < segLabels.length; s++) {
    ...
    // reach(연관 폐포)로 제한 — collectChainFlow 의 전역 walk 가 끌어온 무관 노드(화면의 다른 외부호출 등) 제외
    const segNodes = nodes.filter(id => segOf.get(id) === s && renderable(id));
...
// 4단계: 경로 드릴(3단계)에서 노드를 눌러 들어온 호출관계분석.
//   3단계와 동일한 경계 흐름(화면/endpoint/외부·인프라만)을 focus 노드 기준으로 보여준다.
//   내부 실행 체인은 하단 프로세스 독, 노드 상세는 우측 패널.
function renderFocusPathFlow() {
  const n = nodeById.get(state.focus);
  const bar = document.getElementById('analysis-bar');
  bar.classList.remove('hidden');
  bar.classList.add('svc-mode', 'no-depth');
  document.getElementById('back-to-browse').textContent = '⟵ ' + (state.fromSvcPath || '전체보기');
  document.getElementById('ab-focus').innerHTML =
    `<span class="ab-focus-label">기준</span> <b>${esc(pickLabelOf(n))}</b>`
    + (n.project ? `<span class="ab-proj">${esc(n.project)} · 연결된 화면·endpoint</span>` : '');
  renderFocusFromPathCrumb(n);
  // 경계 흐름: focus 노드 기준. 다른 노드 클릭 시 그 노드로 재중심(경로 트레일 유지).
  const trail = { svc: state.fromService, path: state.fromSvcPath, repo: state.fromSvcRepo };
  renderBoundaryFlow(n.project || state.fromService, [state.focus], id => setFocusFromPath(id, trail));
}
...
function makeComponentCard(sup, info, showSvc) {
  ...
  card.addEventListener('click', () => info.eps ? setSvcPath(info.svc, info.ck, repoOf(info.svc)) : setService(info.svc));
...
// 4단계 브레드크럼: 전체보기 › [repo] › svc › 경로키 › 🎯 노드  (3단계 경로 드릴에서 들어온 호출관계분석)
//   각 단계는 제자리로 복귀: repo→repo 포커스, svc→(repo===svc면 repo 포커스/아니면 서비스 보기), 경로키→3단계 드릴.
function renderFocusFromPathCrumb(n) {
  const svc = state.fromService, path = state.fromSvcPath, repo = state.fromSvcRepo;
  const bc = document.getElementById('breadcrumb');
  bc.style.display = 'flex';
  let html = `<a class="bc-link" id="bc-ov">🗺️ 전체보기</a><span class="bc-sep">›</span>`;
  if (repo && repo !== svc)
    html += `<a class="bc-link" id="bc-repo">${svcBadge(repo)}</a><span class="bc-sep">›</span>`;
  html += `<a class="bc-link" id="bc-svc">${svcBadge(svc)}</a><span class="bc-sep">›</span>`
    + `<a class="bc-link" id="bc-path">${esc(path)}</a><span class="bc-sep">›</span>`
    + `<span class="bc-focus">🎯 ${esc(n.method || n.externalUrl || state.focus)} 호출관계분석</span>`;
  bc.innerHTML = html;
  bc.querySelector('#bc-ov').addEventListener('click', () => setOverview(true));
  bc.querySelector('#bc-repo')?.addEventListener('click', () => setOverview(true, repo));
  bc.querySelector('#bc-svc').addEventListener('click', () =>
    repo && repo === svc ? setOverview(true, repo) : setService(svc));
  bc.querySelector('#bc-path').addEventListener('click', () => setSvcPath(svc, path, repo));
}
...
function renderAnalysis() {
  ...
  document.getElementById('back-to-browse').textContent =
    state.fromSvcPath ? '⟵ ' + state.fromSvcPath : state.fromService ? '⟵ ' + state.fromService : '⟵ 전체보기';
  document.getElementById('ab-focus').innerHTML =
  ...
  if (state.fromService && state.fromSvcPath) renderFocusFromPathCrumb(n);   // 4단계: 전체보기›[repo]›svc›경로›노드
...
function navigateSearchItem(el) {
  ...
  switch (a.t) {
    case 'focus':     if (!nodeById.has(a.id)) return; setFocusFromPath(a.id, focusPathCtxOf(nodeById.get(a.id))); break;   // 검색 → 4단계 직행(노드에서 경로 트레일 도출)
...
function attachHandlers() {
  ...
  document.getElementById('back-to-browse').addEventListener('click', () => {
    ...
    else if (state.fromService && state.fromSvcPath) setSvcPath(state.fromService, state.fromSvcPath, state.fromSvcRepo);   // 4단계 → 3단계 경로 드릴
...
// =========================================================================
const FEATURE_VER = '107';                      // 기능 모듈 캐시 버스팅
